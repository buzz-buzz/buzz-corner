...
module.exports = {
  async redirect(isMobile, code, base64_callback_origin, base64_query_string, redirect) {
    function redirectFail(msg) {
      redirect(`/wechat/oauth/fail/${encodeURIComponent(new Buffer(encodeURIComponent(msg)).toString('base64'))}?callback_origin=${base64_callback_origin}&base64_query_string=${base64_query_string}`)
    }
    try {
        ...
        if (json.errcode || json.errmsg) {
            ...
            redirect(`/wechat/oauth/success/${encodeURIComponent(new Buffer(encodeURIComponent(userInfoResponse)).toString('base64'))}?callback_origin=${base64_callback_origin}&base64_query_string=${base64_query_string}`);
        }
    } catch (e) {
        console.error(e);
        redirectFail(e)
