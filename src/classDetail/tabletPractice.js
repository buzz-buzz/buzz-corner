...
export default class Practice extends React.Component {
    ...
    async reReplyButtonClicked(buttonIndex = this.state.replies.length - 1) {
        ...
        await this.state.replies[buttonIndex].tabletAudio.startRecording();
    }
    ...
    async replyButtonClicked(buttonIndex = this.state.replies.length - 1) {
        ...
        if (!this.state.replies[buttonIndex].answered) {
            ...
            //todo
            this.state.replies[buttonIndex].tabletAudio.play();
    ...
    async endReply() {
        ...
        if (this.state.currentReplying < this.state.replies.length - 1) {
            await this.state.replies[this.state.replies.length - 1].tabletAudio.stopRecording();
            return;
        }

        if (this.props.audioUpload) {
            try {
                let url = await this.state.replies[this.state.replies.length - 1].tabletAudio.stopRecordingWithQiniuLink();
