...
ath.Class = function (options) {
	...
	if ( this.options.detectHomescreen ) {
		...
		if ( this.options.detectHomescreen === 'hash' ) {
			window.history.replaceState('', window.document.title, document.location.href + '#ath');
		} else if ( this.options.detectHomescreen === 'smartURL' ) {
            window.history.replaceState('', window.document.title, document.location.href.replace(/(\/)?$/, '/ath$1'));
		} else {
            window.history.replaceState('', window.document.title, document.location.href + (document.location.search ? '&' : '?' ) + 'ath=');
		}
	}
...
ath.Class.prototype = {
	...
	updateViewport: function () {
		...
		var screenWidth = ath.OS === 'ios' ? this.orientation === 'portrait' ? window.screen.width : window.screen.height : window.screen.width;
		this.scale = window.screen.width > clientWidth ? 1 : screenWidth / window.innerWidth;
...
function _removeToken () {
	...
	if ( _reSmartURL.test(window.location.href) ) {
        window.history.replaceState('', window.document.title, window.location.href.replace(_reSmartURL, '$1'));
	}

	if ( _reQueryString.test(window.location.search) ) {
        window.history.replaceState('', window.document.title, window.location.href.replace(_reQueryString, '$2'));
	}
}
